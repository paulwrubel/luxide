...
export function createTriangleMesh(
  ...
  const vertices = new Float32Array([
    a[0], a[1], a[2], // vertex a
    b[0], b[1], b[2], // vertex b
    c[0], c[1], c[2], // vertex c
  ]);
...
export function createParallelogramMesh(
  ...
  const ll = lower_left;
  const vertices = new Float32Array([
    ll[0],           ll[1],           ll[2],           // lower left
    ll[0] + u[0],    ll[1] + u[1],    ll[2] + u[2],    // lower right
    ll[0] + v[0],    ll[1] + v[1],    ll[2] + v[2],    // upper left
    ll[0] + u[0] + v[0], ll[1] + u[1] + v[1], ll[2] + u[2] + v[2], // upper right
  ]);
