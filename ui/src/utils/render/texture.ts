...
export type RawTextureData = RawTextureChecker | RawTextureImage | RawTextureSolidColor;

export function isTextureData(data: unknown): data is TextureData {
  return isTypedObject(data) && ['checker', 'image', 'color'].includes(data.type);
}
...
export function getTextureDataSafe(
  ...
  try {
    ...
    console.warn(`Failed to get texture data (${nameOrData}), using default texture`, e);
    return {
...
export type NormalizedTextureChecker = Omit<RawTextureChecker, 'even_texture' | 'odd_texture'> & {
  even_texture: string;
  odd_texture: string;
};
